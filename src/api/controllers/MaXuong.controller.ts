import { MaXuong } from '@/api/schemas/MaXuong.schema'
import * as service from '@/api/services/MaXuong.service'
import { RequestBodyType } from '@/type'
import { NextFunction, Request, Response } from 'express'

const NAMESPACE = 'controllers/ma-xuong'

export const createNewItem = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const dataRequest: MaXuong = {
      ...req.body
    }
...
export const getItemByPk = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const maXuong = String(req.params.maXuong)
    const itemFound = await service.getItemByPk(maXuong)
...
export const updateItemByPk = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const maXuong = String(req.params.maXuong)
    const itemToUpdate: MaXuong = {
      ...req.body
    }
    const itemUpdated = await service.updateItemByPk(maXuong, itemToUpdate)
...
export const deleteItemByPk = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const maXuong = String(req.params.maXuong)
    const destroyed = await service.deleteItemByPk(maXuong)
